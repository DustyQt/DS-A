...
class Forecast(Resource):
    def post(self):
        ...
        if args['model']== 'linear_regression':
            ...
            window=min(14,math.floor(len(data)/2))
            for i in range(window):
                X= np.arange(window).reshape(-1,1)
                Y= data.iloc[i:i+window]
                model = linear_model.LinearRegression().fit(X, Y)
                linear_model.LinearRegression(copy_X=True, fit_intercept=True, n_jobs=1, normalize=False)
                Y_pred = model.predict(X[window-1].reshape(-1,1))
                print(window)
                serie=pd.Series({'Real':data.iloc[i+window]['promedio'],'Expected':Y_pred[0][0]},name=data.index.values[i+window])
